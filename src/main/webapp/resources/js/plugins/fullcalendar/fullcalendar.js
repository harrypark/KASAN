...
var DayGrid = FC.DayGrid = Grid.extend(DayTableMixin, {
	...
	renderNumberCellHtml: function(date) {
		...
		//html +='<button type="button"  id="'+date.format()+'"  class="evtModal btn btn-default btn-xs pull-right">Add/Edit</button>';
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
		...
		if (seg.isStart) {
			...
			if (timeText) {
				timeHtml = '<span class="fc-time">test' + htmlEscape(timeText) + '</span>';
			}
		}
			...
			'>' +
				'<div class="fc-content">' +
					(timeText ?
						...
						' data-full="' + htmlEscape(fullTimeText.replace(' -','분 -')) + '"' +
						'>' +
							'<span>' + htmlEscape(timeText.replace(' -','분 -')) + '</span>' +
						'</div>' :
						''
						) +
					(event.title ?
